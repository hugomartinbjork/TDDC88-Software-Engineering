...
def create_transaction(
    ...
    if id is None: id = "1337"
    if storage_unit is None: storage_unit = create_storageunit()
    if by_user is None: by_user = create_user()
    if article is None: article = create_article()
    if time_of_transaction is None: time_of_transaction = datetime.now()
...
def create_storageunit(id = "1337", 
    name = "testStorageUnit", building = "testbuilding",
    floor = "testFloor", costCenter = None):

    if costCenter is None: costCenter = create_costcenter()
    storage_unit = StorageUnit(name=name, building=building,
        floor=floor, cost_center=costCenter)
