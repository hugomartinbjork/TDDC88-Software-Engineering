...
class StorageServiceEconomyTest(TestCase):
    def setUp(self):
        transactedarticle = create_article(price=10)
        cost_center = create_costcenter(id="123")
        storage_unit = create_storageunit(costCenter=cost_center)
        transactiontime = datetime.date(2000,7,15)
        transactionlist = []
        transactionlist.append(create_transaction(article=transactedarticle, amount=2, operation=2, storage_unit=storage_unit, time_of_transaction=transactiontime))
        transactionlist.append(create_transaction(article=transactedarticle, amount=2, operation=1, storage_unit=storage_unit, time_of_transaction=transactiontime))
        transactionlist.append(create_transaction(article=transactedarticle, amount=4, operation=1, storage_unit=storage_unit, time_of_transaction=transactiontime))
        storage_access_mock = storageAccess
        storage_access_mock.get_transaction_by_storage = MagicMock(return_value=transactionlist)
